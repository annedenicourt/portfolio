...
import { woods } from '../datas/dataList'
import Lightbox from 'react-image-lightbox';
import 'react-image-lightbox/style.css';

export default function CardWood() {

    const [photoIndex, setPhotoIndex] = useState(0);
    const [isOpen, setIsOpen] = useState(false);
    const handleShow = () => setIsOpen(true);
    const handleClose = () => setIsOpen(false);
    const images = woods.map(function (wood) {return wood.image;});

    return (  <>
        <div className="row mx-0 mb-5 text-center d-flex flex-column flex-md-row justify-content-center">
            <h3 className="mt-3 mb-3">TOURNAGE SUR BOIS</h3>
